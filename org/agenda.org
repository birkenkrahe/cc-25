...
* Week 14: Pointers to memory
#+attr_html: :width 400px :float nil:
[[../img/pointers.png]]

- [ ] There will be one more test (on pointers)
- [ ] There will be one more programming assignment (with pointers)
- [ ] There will be one more bonus programming assignment
- [ ] I will drop your worst test result and 1 programming assignnment

** TODO Review 1: Pointers (creation to initialization)
...
** TODO Review 2: Pointers (pass by value or reference)

1. What is the result of passing a variable to a function by value?
   #+begin_quote
   When a variable is passed by value, the function receives a *copy* of
   the original variable.  Any changes made to the parameter inside
   the function do *not* affect the original variable in the calling
   function.
   #+end_quote

2. What does the pointer declaration `int *p = &i;` mean?
   #+begin_quote
   This means that `p` is declared as a pointer to an integer, and it is initialized to store 
   the *address* of the variable `i`. So `p` does not contain the value of `i`, but points to 
   the memory location where `i` is stored.
   #+end_quote

3. What happens when you pass an array to a function in C?
   #+begin_quote
   When an array is passed to a function, it *decays* into a pointer to its first element. 
   That means the function actually receives a pointer to the beginning of the array, 
   and not a copy of the array itself.
   #+end_quote

4. Why is dynamic memory allocation important?
   #+begin_quote
   Dynamic memory allocation lets a program request memory at *runtime* rather than compile time. 
   This is essential for working with data whose size is not known in advance. It provides 
   flexibility and efficient memory use, though it requires manual management (e.g., `malloc`, `free`).
   #+end_quote

5. In C, what is the difference between `*p = 2;` and `p = 2;` when `p` is an integer pointer?
   #+begin_quote
   `*p = 2;` means: "store the value 2 in the memory location pointed to by `p`."  
   `p = 2;` means: "make the pointer `p` point to memory address 2," which is almost always 
   invalid and dangerous unless you're explicitly working with that address. The first uses 
   *indirection*, the second reassigns the pointer.
   #+end_quote


* Week 15: Structures - C's classes
#+attr_html: :width 400px :float nil:
#+
[[../img/structures.png]]

At the end of this week, you should know if you need to write the
...
- This is the beginning of object-oriented programming.
