...
* Week 16: Transitioning to C++
#+attr_html: :width 400px :float nil:
#+caption: The guy in the picture is Bjarne Stroustrup
[[../img/cpp.png]]

- We're going to look at the C++ version of several examples.
- This will show you some differences between C and C++.
- It will be a preparation for CSC 240 (Data structures with C++).
- We'll also discuss what you could do in the summer to prepare.

** TODO Review: C ~struct~

1. What is a ~struct~ in C and why would you use it? 
   #+begin_quote
   A ~struct~ groups related variables (e.g., coordinates, player stats)
   into a single data type to improve code organization and
   readability.
   #+end_quote
...
4. How would you store and initialize 1000 ~struct Point~ values
   efficiently?
   #+begin_quote
   Use an array: ~struct Point points[1000];~ and loop to initialize
   each ~points[i]~.
   #+end_quote
...
5. Why is it better to pass a pointer to a ~struct~ rather than the
   ...
   memory and improves performance for large data.
   #+end_quote
...
6. What is the difference between ~.~ and ~->~ when accessing ~struct~
   ...
   #+begin_src C 
     ...
     printf("Jack is a %d old %cale of %d cm height.\n",
            Jack.age, Jack.sex, Jack.height);

     struct Person *aboutJack = &Jack;
     printf("Jack is a %d old %cale of %d cm height.\n",
            aboutJack->age, aboutJack->sex, aboutJack->height);
   #+end_src
...
1. What does a constructor do in C++?
   #+begin_quote
   It automatically initializes a new object's state when it's
   created.
   #+end_quote

2. Why is it useful to define a destructor?
   #+begin_quote
   It ensures resources are cleaned up properly when the object is
   destroyed.
   #+end_quote

3. What is the `this` pointer?
   #+begin_quote
   A pointer to the current object instance, used to access its own
   members.
   #+end_quote

4. What are static members?
   #+begin_quote
   Members shared across all instances of a class, useful for counting
   or shared state.
   #+end_quote

5. Why use initializer lists in constructors?
   #+begin_quote
   They are efficient and preferred for initializing member variables
   directly.
   #+end_quote
    
** TODO Summary and Outlook
#+attr_html: :width 200px :float nil:
[[../img/333.png]]

- 33 Programming assignments
- 300 Test questions
- 333 Lines of code written by every student (at least)

You're now ready for more, bigger and better things!

*Free book:* [[https://beej.us/guide/bgc/html/split/index.html][Beej's Guide to C Programming]] (free) - v0.9.24 April 6, 2025,
online at beej.us/guide/bgc - gateway to system programming.

*Free book:* [[https://rooksguide.org/wp-content/uploads/2013/12/rooks-guide-isbn-version.pdf][Rook's Guide to C++]] (free) - see rooksguide.org. Quick
...
*Non-free book*: Stroustrup - Programming (3rd edition, 2024), check out
[[https://stroustrup.com/programming.html][stroustrup.com/programming.html]] or "[[https://stroustrup.com/tour3.html][A Tour of C++]]" for full immersion.

*Competitive programming*: Join us for a 1 hr per week seminar solving
programming problems in any language you like (C++, Python, Java...) -
CSC 450, Thursday 4 pm, Derby 209.

*Data Structures (with C++*): The next course in the series, for
...
*Introduction to Programming with Java*: Yet another important language
that dominates in industry applications, CSC 115 MWF 1000-1050 am

*Literate Programming*: In all my upper-level classes, you'll work with
the Emacs + Org-mode IDE (instead of OneCompiler). There's a steep
learning curve. I'll give you something to do before term starts.
